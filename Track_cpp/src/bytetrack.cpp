...
cv::Mat static_resize(cv::Mat& img) {
    ...
    cv::Mat out(INPUT_H, INPUT_W, CV_8UC3, Scalar(114, 114, 114));
...
// float* blobFromImage_origin(Mat& img){
...
static void decode_outputs(float* prob, vector<Object>& objects, float scale, const int img_w, const int img_h) {
        ...
        std::cout << "input : " << INPUT_W <<','<< INPUT_H<<std::endl;
...
int main(int argc, char** argv) {
	...
	while (true)
    {
        ...
        float scale = min(INPUT_W / (img_w * 1.0), INPUT_H / (img_h * 1.0));
