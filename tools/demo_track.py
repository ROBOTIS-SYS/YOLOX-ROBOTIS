...
def imageflow_demo_track(predictor, vis_folder, current_time, args):
...
def main(exp, args):
    ...
    if not args.trt:
        if args.ckpt is None:
            ckpt_file = os.path.join(file_name, "model_trt.pth")
        else:
            ckpt_file = args.ckpt
        logger.info("loading checkpoint")
        ckpt = torch.load(ckpt_file, map_location="cpu")
    ...
    if args.demo == "image":
        ...
        # imageflow_demo_track(predictor, vis_folder, current_time, args)
        imageflow_demo_track(predictor, vis_folder, current_time, args)
