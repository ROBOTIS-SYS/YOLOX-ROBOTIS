...
class TRTWrapper():
    ...
    def load_model(self):
        ...
        with open(self.model_path, 'rb') as f:
            # trt 모델을 읽어 serialized ICudEngine을 deserialized함
            self.engine = runtime.deserialize_cuda_engine(f.read())

        # ICudEngine을 이용해 inference를 실행하기 위한 context class생        assert self.engine
    ...
    def inference(self, input_image):
        ...
        # input image array(host)를 GPU(device)로 보내주는 작업
        ...
        for o in range(len(self.outputs)):
            cuda.memcpy_dtoh(self.outputs[o]['host_allocation'],
                             self.outputs[o]['allocation'])  # GPU에서 작업한 값을 host로 보냄

        # print(self.outputs[0]['output'])
        outputs = self.outputs[0]['host_allocation']  # detection된 object개수
        # outputs = self.outputs[1]['host_allocation']  # detection된 object coordinate
        bbox = outputs[:, :, :4]
        obj = outputs[:, :, 4]
        cls_score = outputs[:, :, 5]

        return [bbox, obj, cls_score]
...
class InferenceRunner():
    ...
    def postprocess(self, prediction, conf_thre=0.7, nms_thre=0.45, class_agnostic=False):
        # box_corner = prediction.new(prediction.shape)
        num_classes = 1
        bbox, obj, cls = prediction
        obj = torch.from_numpy(obj).unsqueeze(2)
        cls = torch.from_numpy(cls).unsqueeze(2)
        box_corner = bbox.copy()
        # bbox_corner = bbox.new(bbox.shape)

        box_corner[:, :, 0] = bbox[:, :, 0] - bbox[:, :, 2] / 2
        box_corner[:, :, 1] = bbox[:, :, 1] - bbox[:, :, 3] / 2
        box_corner[:, :, 2] = bbox[:, :, 0] + bbox[:, :, 2] / 2
        box_corner[:, :, 3] = bbox[:, :, 1] + bbox[:, :, 3] / 2
        bbox[:, :, :4] = box_corner[:, :, :4]
        bbox = torch.from_numpy(bbox)

        class_conf, class_pred = torch.max(cls, 2, keepdim=True)

        conf_mask = (obj * class_conf >= conf_thre).squeeze()
        # output = [None for _ in range(len(prediction))]

        # Detections ordered as (x1, y1, x2, y2, obj_conf, class_conf, class_pred)
        detections = torch.cat((bbox, obj, class_conf, class_pred.float()), 2)
        detections = detections[0][conf_mask]
    ...
    def visual(self, output, img_info, cls_conf=0.35):
        ...
        vis_res = vis(img, bboxes, scores, cls, cls_conf, "human")
        return vis_res
    ...
    def run(self):
        for i, filename in enumerate(os.listdir(self.img_folder)):
            ...
            preproc_image, ratio = self.preproc(img, input_size)
            img_info["ratio"] = ratio

            import torch
            preproc_image = torch.from_numpy(preproc_image).unsqueeze(0)
            preproc_image = preproc_image.float()

            # inference
            inf_res = self.model.inference(preproc_image)
            inf_res = self.postprocess(inf_res)

            vis_res = self.visual(inf_res, img_info)
    ...
    def print_result(self, result_image, result_label, count, save_path):
        ...
        for i in range(int(num_detections)):
            ...
            result_image = cv2.rectangle(result_image, (x1, y1-5-text_h),
                                         (x1+text_w, y1), color, -1)
            result_image = cv2.putText(result_image, str(
                detected), (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_folder', required=True,
                        default='inference/images', help='image path')
    ...
    parser.add_argument('--device', required=False, default='GPU', help='CPU or GPU for openvino')
    parser.add_argument('--classes', required=True, default='./coco.yaml',
                        help='yaml file with class info')
